...
with tempfile.TemporaryDirectory(prefix='sentinel-browser-') as d:
    base=Path(d);source=base/'app.log'
    ...
    async def model(payload,**kwargs):
        ...
        return {'findings':[{'title':'Pool de conexiones agotado','summary':'La aplicación no dispone de conexiones.','severity':'HIGH','category':'reliability','evidence_ids':[e['id']],'reasoning':'Evidencia sintética de prueba.','next_steps':'Comprobar conexiones activas.'}]}
    ...
    try:
        with sync_playwright() as pw:
            ...
            page.get_by_role('button',name='No notificar este problema',exact=True).click()
            page.get_by_role('button',name='Guardar',exact=True).click()
            page.get_by_text('Configuración guardada.',exact=True).wait_for()
        ...
        print('PASS: login, machine, file import, two-pass analysis, evidence, mute, destination and responsive layout')
    finally:
