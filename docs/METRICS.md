...
The collector reads CPU utilization and I/O wait, RAM availability, swap occupancy, 1/5/15-minute load, uptime, free disk space and free inodes. Disk paths start with `/`; mounted local devices are now detected by default (up to 16 distinct devices/mount paths). Discovery excludes loop devices, Snap/SquashFS and read-only image/media mounts; their full images are not writable-capacity problems. The root filesystem is still included, including on container hosts. Disable discovery to use only the paths you configure. Remote paths and discovery belong to the sender configuration. CPU percentage requires two counter reads, excludes I/O wait and does not double-count guest CPU time. RAM uses `MemAvailable`, not `MemFree`; unconfigured swap and unavailable readings are omitted rather than invented as zero. Disk usage includes reserved space unavailable to the portal user. Source definitions follow the [Linux kernel `/proc` documentation](https://docs.kernel.org/filesystems/proc.html). Container readings may describe the host rather than container limits; cgroup-specific monitoring is not implemented.
...
## Memory, CPU threads and disk details

RAM shows total, used (`MemTotal - MemAvailable`), free (`MemFree`), shared (`Shmem`), buffers, cache (`Cached + SReclaimable`), buff/cache and available. These fields overlap: available memory includes reclaimable memory, and shared memory is not an additional allocation to add to the others. The names follow [procps `free`](https://gitlab.com/procps-ng/procps/-/raw/master/man/free.1). Older stored or remote samples display missing fields as unknown until the sender is updated.

**CPU threads** shows each logical CPU on its own 0–100% scale, after two counter reads, with up to 256 threads per sample. Aggregate CPU remains the average workload across logical CPUs. Disk bars distinguish allocated space, reserved free space and space available to the service user; the warning percentage includes reserved unavailable space. Device and filesystem names appear when known. Recent and hourly plots share time/value axes, gaps, inspection controls and explicit units.

**More disk info** is an optional local-host action. Opening its dialog does not scan; **Calculate largest folders** starts a bounded `du -x -B1 --max-depth=1 --null -- PATH` subprocess without a shell or elevated privileges. It lists up to 30 largest top-level directories, with a 15-second time budget and bounded output. It stays on one filesystem and does not follow links. Permission failures, timeout and interruption produce an explicit partial result; an incomplete list can miss large directories. Loose files, snapshots and inaccessible files explain differences from filesystem capacity. Disk activity occurs only when requested, and no LLM call is made. Imported logs cannot enable a scan of the remote machine's filesystem.

Update the receiving portal before updating remote metrics senders, so it accepts the new optional fields. Pausing a machine stops both log and metric ingestion without disabling their saved source configuration.
