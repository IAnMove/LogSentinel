...
async def forward(path, receiver, source_id, token, directory, once=False):
    ...
    if source is None:
        ...
        raise ValueError(
            "This spool belongs to another path; choose a different spool directory"
        )
    try:
        async with httpx.AsyncClient(
            ...
            while True:
                ...
                if rows:
                    ...
                    size = 0
                    for e in rows:
                        item = {"id": e["id"], "raw": e.get("raw", e["message"])}
                    ...
                    try:
                        ...
                        if result.get("status") != "durable" or set(
                            ...
                            raise ValueError(
                                "Receiver did not acknowledge the complete batch"
                            )
                        store.mark(ids, "sent")
                        store.discard_sent()
