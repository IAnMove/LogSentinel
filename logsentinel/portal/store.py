...
class Store:
    def __init__(self, directory):
        ...
        with self.connect() as db:
            ...
            if version and version[0] != '1':
                raise RuntimeError('Unsupported schema version; restore with a compatible version')
            db.execute("INSERT OR IGNORE INTO meta VALUES('schema_version','1')")
            db.execute("INSERT OR IGNORE INTO meta VALUES('settings',?)", (dumps(Settings().model_dump()),))
            db.execute("INSERT OR IGNORE INTO meta VALUES('admin_token',?)", (secrets.token_urlsafe(32),))
    ...
    def audit(self,action,object_id='',detail=''):
        with self.connect() as db:
            db.execute('INSERT INTO audit(created,action,object_id,detail) VALUES(?,?,?,?)',(time.time(),action,object_id,detail[:1000]))

    def objects(self,kind):
        with self.connect() as db:
            return [dict(json.loads(r['data']),id=r['id']) for r in db.execute('SELECT * FROM objects WHERE kind=? ORDER BY created',(kind,))]

    def get(self,kind,id):
        with self.connect() as db:
            r=db.execute('SELECT data FROM objects WHERE id=? AND kind=?',(id,kind)).fetchone()
            return dict(json.loads(r[0]),id=id) if r else None

    def put(self,kind,data,id=None):
        id=id or uid()
        with self.connect() as db:
            db.execute('INSERT INTO objects VALUES(?,?,?,?) ON CONFLICT(id) DO UPDATE SET data=excluded.data', (id,kind,dumps(data),time.time()))
            if kind=='destination' and not data.get('enabled'):
                db.execute("UPDATE deliveries SET status='cancelled' WHERE destination_id=? AND status IN ('pending','retry')",(id,))
    ...
    def cursor(self,source_id,path):
        with self.connect() as db:
            row=db.execute('SELECT data FROM cursors WHERE source_id=? AND path=?',(source_id,path)).fetchone()
    ...
    def _metric(db,source,key,value):
        db.execute('INSERT INTO metrics VALUES(?,?,?) ON CONFLICT(source_id,key) DO UPDATE SET value=value+excluded.value',(source,key,value))

    def size(self):
        return sum(p.stat().st_size for p in self.directory.glob('sentinel.db*') if p.is_file())
    ...
    def ingest(self,source,entries,cursor_path=None,cursor=None):
        ...
        with self.connect() as db:
            ...
            for item in entries:
                origin=item['origin']
                if origin in seen or db.execute('SELECT 1 FROM events WHERE source_id=? AND origin=?',(source['id'],origin)).fetchone():
                    continue
                seen.add(origin)
                unique.append(dict(item,id=uid()))
            if unique:
                raw=dumps(unique).encode()
                if self.size()+len(raw)*2 > self.settings().disk_limit_mb*1024*1024:
                    raise OSError('Storage quota reached; incoming data was not acknowledged')
                sid=uid(); now=time.time()
                db.execute('INSERT INTO segments VALUES(?,?,?,?,?,?)',(sid,source['id'],now,len(raw),gzip.compress(raw,mtime=0),hashlib.sha256(raw).hexdigest()))
                for i,item in enumerate(unique):
                    db.execute('INSERT INTO events VALUES(?,?,?,?,?,?,?,?,?,?)',(item['id'],source['id'],source['machine_id'],sid,i,now,item.get('timestamp',''),item.get('service','unknown'),'pending',item['origin']))
                self._metric(db,source['id'],'events_ingested',len(unique))
                self._metric(db,source['id'],'logical_bytes',sum(len(e.get('raw',e.get('message','')).encode()) for e in unique))
            if cursor_path is not None:
                db.execute('INSERT OR REPLACE INTO cursors VALUES(?,?,?)',(source['id'],cursor_path,dumps(cursor)))
        return len(unique)

    def events(self,machine_id='',source_id='',status='',limit=100,offset=0,ids=None):
        query='SELECT * FROM events WHERE 1=1'; params=[]
        for key,value in [('machine_id',machine_id),('source_id',source_id),('status',status)]:
            if value:
                query+=' AND '+key+'=?';params.append(value)
        if ids is not None:
            if not ids: return []
            query+=' AND id IN ('+','.join('?' for _ in ids)+')';params.extend(ids)
        query+=' ORDER BY received,rowid LIMIT ? OFFSET ?';params.extend([min(limit,5000),offset])
        with self.connect() as db:
            rows=db.execute(query,params).fetchall();cache={};out=[]
            for row in rows:
                sid=row['segment_id']
                if sid not in cache:
                    segment=db.execute('SELECT data,sha FROM segments WHERE id=?',(sid,)).fetchone()
                    if segment is None: continue
                    raw=gzip.decompress(segment['data'])
                    if hashlib.sha256(raw).hexdigest()!=segment['sha']: raise ValueError('Segment checksum mismatch')
    ...
    def mark(self,ids,status):
        if not ids:return
        with self.connect() as db:
            db.executemany('UPDATE events SET status=? WHERE id=?',[(status,i) for i in ids])

    def rows(self,table,limit=100):
        if table not in ('jobs','problems','usage','deliveries','audit','revisions'):raise ValueError('Unknown table')
        with self.connect() as db:
            return [dict(r) for r in db.execute(f'SELECT * FROM {table} ORDER BY rowid DESC LIMIT ?', (min(limit,1000),))]
    ...
    def problem(self,id):
        with self.connect() as db:
            ...
            refs=[r[0] for r in db.execute('SELECT event_id FROM appearances WHERE problem_id=? LIMIT 100',(id,))]
            result['evidence']=self.events(ids=refs)
            return result

    def record_usage(self,job,machine,sources,kind,start,input_tokens,output_tokens,status,detail):
        with self.connect() as db:
            db.execute('INSERT INTO usage VALUES(?,?,?,?,?,?,?,?,?,?,?)',(uid(),job,machine,dumps(sources),kind,time.time(),input_tokens,output_tokens,time.monotonic()-start,status,dumps(detail)))
    ...
    def recover(self):
        with self.connect() as db:
            db.execute("UPDATE jobs SET status='retry',error='Interrupted during analysis',updated=? WHERE status='running'",(time.time(),))
            db.execute("UPDATE deliveries SET status='unknown',error='Interrupted during delivery' WHERE status='sending'")

    def prune(self):
        cutoff=time.time()-self.settings().retention_days*86400
        with self.connect() as db:
            ids=[r[0] for r in db.execute('SELECT id FROM segments WHERE created<?',(cutoff,))]
            for sid in ids:
