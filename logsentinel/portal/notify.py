...
def enqueue(store, problem_id):
    ...
    for rule in store.objects("rule"):
        if rule["action"] != "mute":
            continue
        try:
            if evidence and all(matches(rule, e, problem_id) for e in evidence):
                muted = True
        except Exception:
            store.set_meta(
                "rule_error:" + rule["id"], "Mute filter failed; not applied"
            )
    payload = {
        "schema_version": 1,
...
class Outbox:
    ...
    async def send(self, dest, payload):
        ...
        if kind == "file":
            ...
            if Path(name).name != name:
                raise ValueError(
                    "File destination must be a filename within notifications/"
                )
            folder = self.store.directory / "notifications"
            folder.mkdir(mode=0o700, exist_ok=True)
            path = folder / name
            if (
                path.exists()
                and path.stat().st_size >= dest.get("rotation_mb", 10) * 1024 * 1024
            ):
                keep = dest.get("keep_archives", 3)
                for i in range(keep, 1, -1):
                    previous = folder / (name + f".{i-1}.gz")
                    if previous.exists():
                        previous.replace(folder / (name + f".{i}.gz"))
        ...
        if kind == "system":
            ...
            try:
                ...
                if proc.returncode is None:
                    ...
                    await proc.wait()
            if proc.returncode:
                raise ValueError("Desktop notification was rejected")
            return "accepted"
        ...
        if kind == "hermes":
            ...
            headers["X-Webhook-Signature-V2"] = hmac.new(
                dest["secret"].encode(), ts.encode() + b"." + raw, hashlib.sha256
            ).hexdigest()
        ...
        if not 200 <= response.status_code < 300:
            raise ValueError(
                f"Destination rejected request (HTTP {response.status_code})"
            )
        ...
        async with self.lock:
            with self.store.connect() as db:
                rows = [
                    dict(r)
                    for r in db.execute(
                        "SELECT * FROM deliveries WHERE status IN ('pending','retry') AND next_try<=? ORDER BY created LIMIT 20",
                        (time.time(),),
                    )
                ]
            for row in rows:
                ...
                if dest and problem:
                    ...
                    if (
                        dest.get("machine_id")
                        and dest["machine_id"] != problem["machine_id"]
                    ):
                        allowed = False
                    if dest.get("source_id") and dest["source_id"] not in {
                        e["source_id"] for e in evidence
                    ...
                    for rule in self.store.objects("rule"):
                        try:
                            if (
                                ...
                                and evidence
                                and all(
                                    matches(rule, e, problem["id"]) for e in evidence
                                )
                            ):
                                allowed = False
                                status = "muted"
                ...
                with self.store.connect() as db:
                    db.execute(
                        ...
                        (
                            ...
                            error,
                            time.time(),
                            time.time() + min(300, 10 * 2 ** row["attempts"]),
                            row["id"],
                        ),
                    )
                if cancelled:
    ...
    async def test(self, dest):
        ...
        payload = {
            ...
            "delivery_id": id,
            "event_type": "test",
            "title": "Prueba de LogSentinel",
            "summary": "Este es un mensaje de prueba solicitado desde el portal.",
        }
        status = "failed"
        error = None
        try:
