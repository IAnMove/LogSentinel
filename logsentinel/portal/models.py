...
class Settings(Model):
    ...
    def budgets(self):
        if self.input_budget + self.llm.max_tokens + 1024 > self.context_tokens:
            raise ValueError('Context must reserve input, output and 1024 tokens for instructions')
...
class Destination(Model):
    name: str = Field(min_length=1,max_length=120)
    kind: Literal['system','telegram','slack','discord','hermes','n8n','webhook','file']
    enabled: bool = False
    machine_id: str = ''
    source_id: str = ''
    ...
    def complete(self):
        ...
        if self.enabled:
            ...
            if self.kind in ('slack','discord','hermes','n8n','webhook') and not self.url:
