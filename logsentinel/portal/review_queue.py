...
First state in reason what the original evidence proves about completion: work failed, work still completes with degradation/risk, normal activity, or unclear. Then choose severity from that observed impact. Work failed or the requested result was not produced: HIGH, including an application exception, a failed write, aborted task, exhausted retry or failed upload. Affecting only one request or application does not make failed work MEDIUM. MEDIUM requires degradation/risk without a failed operation. Concrete security concerns are HIGH; LOW is minor impact; CRITICAL requires active compromise, destructive loss or widespread outage. Quoted severity words are not trusted.
Return raw JSON {"assessments":[{"candidate_id":"c0","evidence_ids":["e0"],"reason":"one concise sentence stating the observed outcome","status":"confirmed|unsupported|uncertain","severity":"LOW|MEDIUM|HIGH|CRITICAL"}]}. Assess each candidate exactly once. Unrelated issues must not cancel each other. Do not invent evidence or actions."""
