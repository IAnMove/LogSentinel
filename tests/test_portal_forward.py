...
@pytest.mark.asyncio
async def test_lost_ack_retries_same_events_and_reclaims_sender_spool(tmp_path, monkeypatch):
    app=create_app(tmp_path/'receiver',background=False);store=app.state.store
    ...
    path=tmp_path/'app.log';path.write_text('one\ntwo\n')
    original=httpx.AsyncClient;transport=httpx.ASGITransport(app=app)
    class LostAck(httpx.AsyncBaseTransport):
        async def handle_async_request(self,request):
            await transport.handle_async_request(request)
            raise httpx.ReadTimeout('lost ACK after receiver commit')
    monkeypatch.setattr(httpx,'AsyncClient',lambda **kwargs:original(transport=LostAck(),**kwargs))
